...
export default function LeaderboardsPage() {
  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-4 gap-2 rounded-lg border border-[#6FB4EB] bg-[#D0FEF5] text-[#4A051C] p-2">
      ...
      <section className="mt-6 grid gap-2">
        {rest.map((row) => (
          <div key={row.rank} className="grid grid-cols-[34px_46px_1fr_auto] items-center gap-3 rounded-lg px-1 py-2">
            <div className="text-lg font-black text-[#D0FEF5]">{row.rank}</div>
            <div className="grid h-10 w-10 place-items-center rounded-full bg-[#094586] text-sm font-black text-[#6FB4EB]">{row.name[0]}</div>
            <div>
